...
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICES_ROLE_KEY;
...
app.post("/generate", async (req, res) => {
  try {
    ...
    if (!user_id || !Array.isArray(ingredients) || ingredients.length === 0) {
      return res.status(400).json({ error: "user_id and ingredients[] required" });
    }
...
`;
    ...
    const { data, error: insertError } = await supaAdmin
      .from("recipes")
      .insert([
        {
          user_id,
          title: titleLine || "simple recipe",
          ingredients: JSON.stringify(ingredients), // or just ingredients if text[]
          recipe_text,
        },
      ])
      .select();
